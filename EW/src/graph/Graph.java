 ...
 * 
 * Children of this class must implement propertyChange and look for
 * TICKS_LOADED_PROPERTY and BARS_LOADED_PROPERTY to have ticks and bars
 * provided.
 * 
...
public abstract class Graph extends ChartView implements InteractiveGraph,
		PropertyChangeListener, OfferListener {
	...
	public static final String TICKS_LOADED_PROPERTY = "Ticks";
	public static final String BARS_LOADED_PROPERTY = "Bars";
	...
	protected Graph(ChartController controller, TimeAxis correspondingTimeAxis,
		...
		if (controller == null || correspondingTimeAxis == null
				|| correspondingRateAxis == null || offerFeed == null)
		...
		this.offerFeed = offerFeed;
		this.timeOfFront = offerFeed.getCurrentTime();
	...
	public OfferSide getOfferSide() {
		return controller.getOfferSide();
	}
	...
	public void setListeningToFeed(boolean listening) {
		if (listening)
			offerFeed.addListener(this);
		else
			offerFeed.removeListener(this);
	}
	...
	protected long getCurrentTime() {
		return offerFeed.getCurrentTime();
	}
	...
	public void setAdjustRatesToFeed(boolean adjustRates) {
		...
		for (PropertyChangeListener pcl : registeredListeners) {
			pcl.propertyChange(new PropertyChangeEvent(this, null,
					isRateAdjustingToFeed, adjustRates));
		}
	...
	public void displayCurrent() {
		long newStartTime = timeOfFront
				- correspondingTimeAxis.getRangeInterval() / 2;
	...
	public void zoom(final double magnification) {
		...
		focusAboutTime(newTimeRange, getCenterTime());
	...
	public void changeTimeRange(long timeToMoveForward) {
		changeTimeRange(correspondingTimeAxis.getStartTime()
				+ timeToMoveForward, correspondingTimeAxis.getEndTime()
				+ timeToMoveForward);
	}
	...
	private void adjustHighAndLowRates() {
		...
		changeRateRange(lowRate - verticalRateMargin, highRate
				+ verticalRateMargin);
	}
	...
	protected void paintComponent(Graphics g) {
		...
		if (propertyChanged || graphicsDependentParametersChanged()) {
			...
			if (getTimeRange().getInterval() < getMinTimeRange())
				focusAboutTime(getMinTimeRange(), getCenterTime());
			else if (getTimeRange().getInterval() > getMaxTimeRange())
				focusAboutTime(getMaxTimeRange(), getCenterTime());
	...
	/**
	 * Returns true if the given instances are equal to their correspondants of
	 * this graph.
	 * 
	 * @return true if the given instances are equal to their correspondants of
	 *         this graph.
	 */
	protected boolean sameProperties(Instrument instrument,
			OfferSide offerSide, Period period) {
		return instrument == getInstrument() && offerSide == getOfferSide()
				&& period == getPeriod();
	}
	...
	/**
	 * Have onTick() be overridden from this class instead of previously from
	 * ITickFeedListener.
	 */
	public abstract void onTick(Instrument instrument, ITick tick);

	/**
	 * Have onBar() be overridden from this class instead of previously from
	 * IBarFeedListener.
	 */
